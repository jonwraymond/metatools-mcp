...
// tokenCache provides thread-safe caching for introspection results.
...
func (c *tokenCache) Get(tokenHash string) *Identity {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *tokenCache) Set(tokenHash string, identity *Identity, ttl time.Duration) {
	c.mu.Lock()
	defer c.mu.Unlock()
